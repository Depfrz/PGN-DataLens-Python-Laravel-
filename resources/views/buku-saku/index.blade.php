        ...
        <form x-data="{ 
            ...
            <div class="relative min-w-[200px]">
                <button @click="open = !open" @click.away="open = false" type="button" 
                    ...
                    <span x-text="selected.length > 0 ? selected.length + ' Tags Dipilih' : 'Filter Tags'"></span>
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex justify-between items-start">
                                        ...
                                        <div class="flex items-center gap-2 flex-shrink-0 ml-2">
                                            @php
                                                $canEdit = (Auth::id() === $doc->user_id) || Auth::user()->hasAnyRole(['Admin', 'Supervisor']) || ($hasWriteAccess ?? false);
                                            @endphp

                                            @if($canEdit)
                                                <a href="{{ route('buku-saku.edit', $doc->id) }}" class="p-1 rounded-full hover:bg-gray-100 text-yellow-600" title="Edit">
                                                ...
                                                <form action="{{ route('buku-saku.destroy', $doc->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus dokumen ini?');" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="p-1 rounded-full hover:bg-gray-100 text-red-600" title="Hapus">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
