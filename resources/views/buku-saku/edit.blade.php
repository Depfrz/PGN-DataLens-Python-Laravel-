<x-buku-saku-layout>
    ...
    <form action="{{ route('buku-saku.update', $document->id) }}" method="POST" enctype="multipart/form-data" id="uploadForm">
        ...
        <!-- File Upload -->
        <div class="mb-6">
            <label class="block text-base font-medium text-gray-700 mb-2">File Dokumen (Opsional)</label>
            <div id="dropZone" class="border-2 border-dashed border-gray-300 rounded-lg p-6 sm:p-10 text-center cursor-pointer hover:bg-gray-50 transition-colors relative">
                ...
                <div id="emptyState" class="{{ $document->file_path ? 'hidden' : '' }}">
                    ...
                    <p class="text-base text-blue-600 font-medium"><span class="hover:underline">Klik untuk ganti file</span> <span class="text-gray-600 font-normal">atau drag & drop</span></p>
                    <p class="text-sm text-gray-500 mt-1">PDF, Word (DOC/DOCX) hingga 30MB. Biarkan kosong jika tidak ingin mengubah file.</p>
                </div>
        ...
        <!-- Title -->
        <div class="mb-6">
            <label for="titleInput" class="block text-base font-medium text-gray-700 mb-2">Judul Dokumen</label>
            <input type="text" name="title" id="titleInput" value="{{ old('title', $document->title) }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 px-4 py-2 text-base" required>
        </div>
        ...
        <div class="mb-6">
            ...
            <div class="relative">
                <input type="date" name="valid_until" id="validUntilInput" value="{{ old('valid_until', $document->valid_until ? $document->valid_until->copy()->subYears(5)->format('Y-m-d') : '') }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 px-4 py-2 text-base cursor-pointer">
            </div>
        ...
        <div class="mb-6">
            ...
            <div class="border border-gray-300 rounded-lg overflow-hidden">
                ...
                <div id="tagsContainer" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 max-h-80 overflow-y-auto p-4 bg-white">
                    @php
